...
install_uefi() {
    ...
    local vendordir
    # since the shim package has not yet been ported to the RISC-V platform, here we
    # directly copy grub as BOOTRISCV64.
    if [ "$arch" = riscv64 ]; then
        vendordir="${target_efi}/EFI/BOOT"
        mkdir -p "${vendordir}"
        cp ${deploy_root}/usr/lib/ostree-boot/efi/EFI/openEuler/grubriscv64.efi ${vendordir}/BOOTRISCV64.EFI
    else
